...
This proposal introduces a top-level configuration setting `.quotaReleaseStrategy` in Kueue's Configuration API (`apis/config/v1beta2`), gated by the `QuotaReleaseStrategy` Feature Gate (Alpha, disabled by default in v0.20). It allows cluster administrators to configure when Kueue releases workload quota reservation during eviction and preemption — either immediately upon deletion initiation / status change (`OnQuotaReleased`) or delayed until all underlying pods reach a terminal phase (`OnTerminal`).
...
- Provide a top-level Configuration API setting `.quotaReleaseStrategy` to control quota release timing across Kueue integrations during eviction and preemption, guarded by the `QuotaReleaseStrategy` feature gate.
- Support `OnQuotaReleased` (default) for fast quota release upon workload eviction.
- Support `OnTerminal` in Alpha (v0.20) for `batch/v1.Job` and `Pod` integrations to delay quota release until underlying pods physically transition to a terminal state (`Succeeded` or `Failed`) upon eviction.
...
- **Default Strategy**: The default strategy is `OnQuotaReleased` across all integrations.
...
- **Opting into `OnTerminal`**: Cluster administrators who require delayed quota release until underlying pods reach terminal phase (`Succeeded`/`Failed`) can explicitly configure `quotaReleaseStrategy: OnTerminal` for supported integrations (`batch/v1.Job`, `Pod`).
...
- Implemented `OnTerminal` strategy support for core `batch/v1.Job` and `Pod` integrations.
